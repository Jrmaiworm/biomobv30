...
import { getEntities as getCadastroSolicitacaos } from 'app/entities/cadastro-solicitacao/cadastro-solicitacao.reducer';
...
export const SolicitacaoItemUpdate = (props: RouteComponentProps<{ id: string }>) => {
  ...
  const cadastroSolicitacaos = useAppSelector(state => state.cadastroSolicitacao.entities);
  ...
  useEffect(() => {
    ...
    dispatch(getItems({}));
    dispatch(getCadastroSolicitacaos({}));
    dispatch(getAcaos({}));
  }, []);
  ...
  const saveEntity = values => {
    const entity = {
      ...
      cadastroSolicitacao: cadastroSolicitacaos.find(it => it.id.toString() === values.cadastroSolicitacao.toString()),
    };
  ...
  const defaultValues = () =>
    isNew
      ...
      : {
          ...
          cadastroSolicitacao: solicitacaoItemEntity?.cadastroSolicitacao?.id,
        };
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="biomobv30App.solicitacaoItem.home.createOrEditLabel" data-cy="SolicitacaoItemCreateUpdateHeading">
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField
                id="solicitacao-item-cadastroSolicitacao"
                name="cadastroSolicitacao"
                data-cy="cadastroSolicitacao"
                label="Cadastro Solicitacao"
                type="select"
              >
                <option value="" key="0" />
                {cadastroSolicitacaos
                  ? cadastroSolicitacaos.map(otherEntity => (
                      <option value={otherEntity.id} key={otherEntity.id}>
                        {otherEntity.id}
                      </option>
                    ))
