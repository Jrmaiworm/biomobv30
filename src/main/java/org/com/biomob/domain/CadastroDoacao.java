...
public class CadastroDoacao implements Serializable {
    ...
    @OneToMany(mappedBy = "cadastroDoacao")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "descricao", "cadastroDoacao", "acao" }, allowSetters = true)
    private Set<DoacaoItem> descricaos = new HashSet<>();
    ...
    public Set<DoacaoItem> getDescricaos() {
        return this.descricaos;
    }

    public void setDescricaos(Set<DoacaoItem> doacaoItems) {
        if (this.descricaos != null) {
            this.descricaos.forEach(i -> i.setCadastroDoacao(null));
        }
        if (doacaoItems != null) {
            doacaoItems.forEach(i -> i.setCadastroDoacao(this));
        }
        this.descricaos = doacaoItems;
    }

    public CadastroDoacao descricaos(Set<DoacaoItem> doacaoItems) {
        this.setDescricaos(doacaoItems);
        return this;
    }

    public CadastroDoacao addDescricao(DoacaoItem doacaoItem) {
        this.descricaos.add(doacaoItem);
        doacaoItem.setCadastroDoacao(this);
        return this;
    }

    public CadastroDoacao removeDescricao(DoacaoItem doacaoItem) {
        this.descricaos.remove(doacaoItem);
        doacaoItem.setCadastroDoacao(null);
        return this;
    }
